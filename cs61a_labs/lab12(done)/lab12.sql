...
CREATE TABLE big AS
  SELECT DISTINCT f.course AS course
  FROM finals f
...
CREATE TABLE remaining AS
  SELECT DISTINCT course, sum(seats)-max(seats) AS remaining
  FROM finals , sizes
  WHERE hall = room
  GROUP BY course;
...
CREATE TABLE pairs AS
    SELECT DISTINCT a.room || " and " || b.room || " together have " ||  (a.seats+b.seats) || " seats" AS rooms
