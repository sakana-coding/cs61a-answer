...
from pooch.utils import temporary_file
...
def accuracy(typed, source):
    ...
    range_max = min(len(typed_words), len(source_words))
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == source[i]:
            ...
            break
    print("Debug", count, total)
    d = {}
    d['id'] = user_id
    d['progress'] = count / total * 1.0
    upload(d)
    return d['progress']
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(timestamps_per_player)):
        for j in range(len(timestamps_per_player[i]) - 1):
            ...
            temporary_list.append(time)
        times.append(temporary_list)
        temporary_list = []
...
def fastest_words(match):
    ...
    for j in range(len(times[0])):
        for i in player_indices:
            temporary_list.append(times[i][j])
        new_times.append(temporary_list)
        temporary_list = []
    new_words = [[],[],[],[],[]]
    ...
    for i in range(len(new_times)):
        for j in range(len(new_times[i])):
            if new_times[i][j] < min:
                min = new_times[i][j]
                index = j
        new_words[index].append(words[i])
        index = 0
        min = 10000
    print_words = []
    for j in player_indices:
        print_words.append(new_words[j])
    return print_words
