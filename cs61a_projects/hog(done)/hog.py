...
def boar_brawl(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 2
    temp = opponent_score // 10 % 10 - player_score % 10
    if 3 * abs(temp) > 1:
        return 3 * abs(temp)
    else:
        return 1
...
def always_roll(n):
    ...
    3
    """
    assert n >= 0 and n <= 10, 'n must be between 0 and 10'
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
...
def make_averaged(original_function, samples_count=1000):
    ...
    def averaged(*args):
        ...
        while i <= samples_count:
            ...
            i += 1
        average = float(sum / samples_count)
        return average
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    temp=boar_brawl(score, opponent_score)+score
    if sus_points(temp)-score>=threshold:
        return 0
    else:
        return num_rolls
