...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        new_expr = Pair(args, body)
        function_name = signature.first
        env.define(function_name, do_lambda_form(new_expr, env))
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    current_expr = bindings
    another_current_expr = bindings
    while current_expr is not nil:
        validate_form(current_expr.first,1,2)
        names = Pair(current_expr.first.first, names)
        current_expr = current_expr.rest
    while another_current_expr is not nil:
        validate_form(another_current_expr.first.rest,1,)
        vals = Pair(scheme_eval(another_current_expr.first.rest.first,env), vals)
        another_current_expr = another_current_expr.rest
...
def do_mu_form(expressions, env):
    ...
    # END PROBLEM 11
def enumerate(expressions, env):
    result = nil
    i = 0
    Pair(Pair(i, expressions.first), result)
    expressions = expressions.rest
    while expressions is not nil:
        result = Pair(result,Pair(i, expressions.first))
        expressions = expressions.rest
        i += 1
    return result
