...
class Ant(Insect):
    ...
    def add_to(self, place):
        assert type(self) is not BodyguardAnt or type(place.ant) is not BodyguardAnt, 'Too many ants in {0}'.format(
            place)
        assert type(self) is not TankAnt or type(place.ant) is not TankAnt, 'Too many ants in {0}'.format(
            place)
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt do noting per turn ,but it has abundant health remaining."""
    name = 'Wall'
    implemented = True
    food_cost = 4
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt eats a bee per turn,but doing nothing in following several turns"""
    name = 'Hungry'
    implemented = True
    food_cost = 4
    chewing_turns = 3
...
class ScubaThrower(ThrowerAnt):
    """A subclass of ThrowerAnt that is waterproof."""
    name = 'Scuba'
    implemented = True
    food_cost = 6
    is_waterproof = True
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while current_place is not None:
            ...
            if type(current_place.ant) is BodyguardAnt and current_place.ant.ant_contained is not None:
                Ant.double(current_place.ant.ant_contained)
            if type(current_place.ant) is TankAnt and current_place.ant.ant_contained is not None:
                Ant.double(current_place.ant)
                Ant.double(current_place.ant.ant_contained)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while current_place is not None and not current_place.is_hive:
            if type(current_place.ant) is BodyguardAnt or type(current_place.ant) is TankAnt:
                dictionary[current_place.ant] = distance
                if current_place.ant.ant_contained is not None:
                    dictionary[current_place.ant.ant_contained] = distance
