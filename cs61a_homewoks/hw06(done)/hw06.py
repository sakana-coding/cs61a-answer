...
class VendingMachine:
    ...
    def restock(self, amount):
        self.stock += amount
        print(f"\'Current {self.goods} stock: {self.stock}\'")
    ...
    def vend(self):
        ...
        if self.funds < self.price:
            ...
            print(f"\'Here is your {self.goods} and ${self.funds - self.price} change.\'")
            self.stock -= 1
            self.funds = 0
...
def store_digits(n):
    ...
    if n < 10:
        ...
        # 计算最高位的数字
        power = 10 ** (count_digits(n) - 1)
        first_digit = n // power
        remaining = n % power
        return Link(first_digit, store_digits(remaining))
