...
def balanced(m):
    ...
    if is_planet(m):
        ...
        assert is_mobile(m), "must get total mass of a mobile or a planet"
        flag1 = balanced(end(left(m))) and balanced(end(right(m)))
        flag2 = total_mass(end(left(m))) * length(left(m)) == length(right(m)) * total_mass(end(right(m)))
        print("Debug:", flag1, flag2)
