...
def interleaved_sum(n, odd_func, even_func):
    ...
    def helper(k):
        if k > n:
            return 0
        current_odd = odd_func(k)
        current_even = 0
        if k + 1 <= n:
            current_even = even_func(k + 1)
        return current_odd + current_even + helper(k + 2)
...
def count_coins(total):
    ...
    def helper(coin, total):
        ...
        if not coin:
            return 0
        return helper(coin, total - coin) + helper(next_smaller_coin(coin), total)
